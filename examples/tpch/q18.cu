...
template <typename JoinMap>
__global__ void probe_l_c_o_join_size(int32_t *lagg_orderkey, int32_t *lagg_quantity, JoinMap o_joinmap, JoinMap c_joinmap, 
  int32_t *o_custkey, int32_t* result_idx, int32_t agg_size
) {
  int32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
...
template <typename JoinMap>
__global__ void probe_l_c_o_join(int32_t *lagg_orderkey, int32_t *lagg_quantity, JoinMap o_joinmap, JoinMap c_joinmap, 
  int32_t *o_custkey,
  multijoin_lco_t *result, int32_t* result_idx, int32_t agg_size
) {
  int32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
  if (tid >= agg_size) return;
  if (lagg_quantity[tid] <= 300) return; // filter by quantity
...
int main(int argc, const char** argv)
{
  ...
  int32_t *l_orderkey, *l_quantity, *o_orderkey, *o_custkey, *c_custkey;
  l_orderkey = read_column_typecasted<int32_t>(lineitem_table, "l_orderkey");
  l_quantity = read_column_typecasted<int32_t>(lineitem_table, "l_quantity");
  ...
  c_custkey = read_column_typecasted<int32_t>(customer_table, "c_custkey");
  ...
  cudaMemcpy(d_l_quantity, l_quantity, sizeof(int32_t) * lineitem_size, cudaMemcpyHostToDevice);
  ...
  probe_l_c_o_join_size<<<getGridSize(lagg_size, TB), TB>>>(
    d_lagg_orderkey,
    d_lagg_quantity,
    l_o_joinmap.ref(cuco::find), c_o_joinmap.ref(cuco::find),
    d_o_custkey,
    d_res_idx, lagg_size
  );
  int32_t lco_joinsize; cudaMemcpy(&lco_joinsize, d_res_idx, sizeof(int32_t), cudaMemcpyDeviceToHost);
  multijoin_lco_t *d_loc_join;
  cudaMalloc(&d_loc_join, sizeof(multijoin_lco_t)*lco_joinsize);
  cudaMemset(d_res_idx, 0, sizeof(int32_t));
  probe_l_c_o_join<<<getGridSize(lagg_size, TB), TB>>>(
    d_lagg_orderkey,
    d_lagg_quantity,
    l_o_joinmap.ref(cuco::find), c_o_joinmap.ref(cuco::find),
    d_o_custkey, d_loc_join,
    d_res_idx, lagg_size
  );
  ...
  double* o_totalprice = read_column<double>(orders_table, "o_totalprice");
