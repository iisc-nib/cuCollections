...
__global__ void k1_build_customer(
  int32_t *c_custkey,
  int8_t *c_mktsegment,
...
__global__ void k2_probe_and_build(
  int32_t *o_orderkey,
  int32_t *o_custkey,
  int32_t *o_orderdate,
  int32_t orderdate_predicate,
...
int main(int argc, const char** argv) {
   ...
   */
   ...
   allocate_to_device(d_c_custkey, c_custkey, customer_size);
   allocate_to_device(d_c_mktsegment, c_mktsegment->column, customer_size);
   int thread_blocks = getGridSize(customer_size, TILE_SIZE);
   k1_build_customer<<<thread_blocks, TB_SIZE>>>(
    d_c_custkey,
    d_c_mktsegment,
    building_predicate,
  ...
  //  std::cout << "Customers filtered: " << c_custkey_map.size() << std::endl; 
    ...
    */
   int32_t *o_orderkey, *o_orderdate, *o_custkey;
   o_orderkey = read_column_typecasted<int32_t>(orders_table, "o_orderkey");
   o_orderdate = read_column<int32_t>(orders_table, "o_orderdate");
   o_custkey = read_column_typecasted<int32_t>(orders_table, "o_custkey");
   int32_t orderdate_predicate = 9204;

  int32_t *d_o_orderkey, *d_o_orderdate, *d_o_custkey;
  allocate_to_device(d_o_orderkey, o_orderkey, orders_size);
  allocate_to_device(d_o_custkey, o_custkey, orders_size);
  allocate_to_device(d_o_orderdate, o_orderdate, orders_size);
  ...
  // cudaMemcpy(&orders_filtered_count, d_orders_filtered_count, sizeof(int32_t), cudaMemcpyDeviceToHost);
  // std::cout << "Total orders filtered: " << orders_filtered_count << std::endl;
  auto o_orderkey_map = cuco::static_map {
   ...
   k2_probe_and_build<<<thread_blocks, TB_SIZE>>>(
    d_o_orderkey,
    d_o_custkey,
    d_o_orderdate,
    orderdate_predicate,
   ...
   */
  int32_t *l_shipdate, *l_orderkey, *o_shippriority;
  double *l_extendedprice, *l_discount;
  l_shipdate = read_column<int32_t>(lineitem_table, "l_shipdate");
  l_orderkey = read_column_typecasted<int32_t>(lineitem_table, "l_orderkey");
  o_shippriority = read_column_typecasted<int32_t>(orders_table, "o_shippriority");
  l_extendedprice = read_column<double>(lineitem_table, "l_extendedprice");
  l_discount = read_column<double>(lineitem_table, "l_discount");

  int32_t *d_l_shipdate, *d_l_orderkey, *d_o_shippriority;
  double *d_l_extendedprice, *d_l_discount;
  allocate_to_device(d_l_shipdate, l_shipdate, lineitem_size);
  allocate_to_device(d_l_orderkey, l_orderkey, lineitem_size);
  allocate_to_device(d_o_shippriority, o_shippriority, orders_size);
  allocate_to_device(d_l_extendedprice, l_extendedprice, lineitem_size);
  allocate_to_device(d_l_discount, l_discount, lineitem_size);

  auto agg_map = cuco::static_map {
    o_orderkey_map.size(),
  ...
  k3_groupjoin<<<thread_blocks, TB_SIZE>>>(
    d_l_shipdate,
    d_l_orderkey,
    d_l_extendedprice,
    d_l_discount,
